...
from vardautomation.tooling import AudioCutter, Mux
...
class Filtering:
    def main(self) -> vs.VideoNode:
        ...
        ivtc = lvsfunc.util.replace_ranges(ivtc, deinterlace, [33926, 34046])
        ivtc = core.std.FreezeFrames(ivtc, [33926-1, 34046-1], [33926, 34046], [33926-1, 34046-1])
        out = ivtc
        ...
        deband = vdf.deband.dumb3kdb(out, threshold=36, grain=[48, 24])
...
if __name__ == '__main__':
    ...
    Patch(X265Encoder('x265_settings'), filtered, TV_NHK, [(33926, 34047)]).run()
    # X265Encoder('x265_settings').run_enc(filtered, TV_NHK)
    # AudioCutter(TV_NHK, track=1).run()
    # Mux(TV_NHK).run()
else:
