...
PATH = r'アズールレーン\[200318]アニメ『アズールレーン』VOLUME 4\BD4\BDMV\STREAM\00000'
SRC = PATH + '.m2ts'
...
OPSTART, OPEND = 1761, 3908
EDSTART, EDEND = 31623, 33783
...
def do_filter():
    ...
    f_1, f_2 = 1010, 2010
    src_inter = src[:EDSTART+f_1] + interpolate[EDSTART+f_1] + src[EDSTART+f_1:EDSTART+f_2]  \
        + interpolate[EDSTART+f_2] + src[EDSTART+f_2:EDEND-2] + src[EDEND:]

    src = depth(src_inter, 16)
    ...
    return final, src_inter


def do_encode(filtered):
    """Compression with x264"""
    print('Qpfile generating')
    vrf.gk(filtered[1], QPFILE)

    print('\n\n\nVideo encode')
    vrf.encode(filtered[0], X264, OUTPUT, **X264_ARGS)
