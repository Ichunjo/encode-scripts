...
JPBD = FileInfo(BDMV_PATH / f'BDMV/STREAM/0000{(NUM - 1) % 3}.m2ts', (0, 34645), preset=(PresetBD, PresetAAC, PresetChapXML))
JPBD.do_qpfile = True
JPBD_NCOP = FileInfo('[BDMV][210127][BRMM-10341][アサルトリリィ BOUQUET 1]/BRMM_10341/BDMV/STREAM/00003.m2ts', (0, -26), preset=PresetBD)
JPBD_NCED = FileInfo('[BDMV][210324][BRMM-10343][アサルトリリィ BOUQUET 3]/BRMM_10343/BDMV/STREAM/00003.m2ts', (None, None), preset=PresetBD)
...
OPSTART, OPEND = 2086, 4242
EDSTART, EDEND = 28987, 31144
...
class Filtering:
    def main(self) -> vs.VideoNode:
        ...
        custom_mask_a = Mask.ass_mask('assault_08_32482_32668_mask.ass', lmask, (32482, 32668))
        lmask = rfs(lmask, custom_mask_a, [(32482, 32667)])
        # -------------------------- SCENEFILTERING AA --------------------------
        aaa = AA.upscaled_sraa(y)
        aa_b = AA.upscaled_sraa(y, 1.5, 3, alpha=0.6, beta=0.2, gamma=40, nrad=2, mdis=20)
        aa_c = AA.upscaled_sraa(y, 1.25, None, 0, alpha=0.99, beta=0.01, gamma=10, nrad=3, mdis=25)
        aa_c = AA.upscaled_sraa(aa_c, 1.15, None, 0, alpha=0.99, beta=0.01, gamma=10, nrad=3, mdis=25)
        aaa = rfs(aaa, aa_b, [(1836, 1910), (10869, 10970), (12612, 12716)])
        aaa = rfs(aaa, aa_c, [(32482, 32667)])
        # -------------------------- SCENEFILTERING AA --------------------------
        ...
        lmask = rfs(lmask, lmask_b, [(22559, 22705)])
        ...
        # return depth(out, 10), lmask, lmask_b, mmmask, custom_mask_a
