...
JPBD_NCED = FileInfo('[BDMV][210224][BRMM-10342][アサルトリリィ BOUQUET 2]/BRMM_10342/BDMV/STREAM/00003.m2ts', (None, None), preset=PresetBD)
...
OPSTART, OPEND = 1870, 4028
EDSTART, EDEND = 32106, 34190
...
class Filtering:
    def main(self) -> vs.VideoNode:
        ...
        if FILTER_ENDCARD:
            ...
            dehalo = MaskedDHA(out, 1.45, 1.45, 0.005, 1.0)
            out = rfs(out, dehalo, [(34527, src.num_frames-1)])
        ...
        # -------------------------- SCENEFILTERING MASK --------------------------
        lmask = rfs(lmask, lmask_b, [(15715, 15750)])
        ...
        deband = rfs(deband, yuv_db_b, [(22351, 22384), (22424, 22452), (30514, 30699),
                                        (30790, 30957), (30988, 31035)])
        # -------------------------- SCENEFILTERING --------------------------
        out = depth(deband, 16)
