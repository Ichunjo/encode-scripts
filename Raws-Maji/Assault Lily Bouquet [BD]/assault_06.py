...
BDMV_PATH = VPath('[BDMV][210224][BRMM-10342][アサルトリリィ BOUQUET 2]/BRMM_10342')

JPBD = FileInfo(BDMV_PATH / f'BDMV/STREAM/0000{(NUM - 1) % 3}.m2ts', [(0, 34647)], preset=(PresetBD, PresetAAC, PresetChapXML))
JPBD.do_qpfile = True
...
class Filtering:
    def main(self) -> vs.VideoNode:
        ...
        import rekt as rkt
        blue_blank = core.std.BlankClip(out, color=[75 << 8, 139 << 8, 127 << 8])
        wtf = rkt.rekt(out, blue_blank, left=230, top=158, right=1680, bottom=918)
        out = rfs(out, wtf, [(7434, 7481)])

        if FILTER_ENDCARD:
            from G41Fun import MaskedDHA
            dehalo = MaskedDHA(out, 1.45, 1.45, 0.25, 1.0)
        ...
        custom_mask_a = Scale.to_gray(core.imwri.Read('assault_06_20517_mask.png'), y)
        custom_mask_a = custom_mask_a.std.Maximum().std.Maximum().std.BoxBlur(0, 2, 2, 2, 2)
        # -------------------------- SCENEFILTERING MASK --------------------------
        # lmask = rfs(lmask, lmask_b, [(20517, 20588)])
        lmask = rfs(lmask, core.std.Expr((mmmask, custom_mask_a), 'x y max'), [(20517, 20588)])
        lmask = rfs(lmask, mmmask, OP_DEBAND + [(17075, 17172), (18191, 18294)])  # type: ignore
        # -------------------------- SCENEFILTERING MASK --------------------------
        ...
        deband = rfs(deband, core.std.MaskedMerge(yuv_db_b, out, lmask), [(27267, 28593), (28666, 28965)])
        # -------------------------- SCENEFILTERING --------------------------
        out = depth(deband, 16)
