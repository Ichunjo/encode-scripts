...
# reader = MplsReader(r'[BDMV]Mushoku Tensei[Vol.2][JP]\BDROM', UNDEFINED)
# reader.write_playlist('chapters', chapters_obj=MatroskaXMLChapters)
# exit()
JPBD = FileInfo(
    r'[BDMV]Mushoku Tensei[Vol.2][JP]\BDROM\BDMV\STREAM\00005.m2ts', 24, -24,
...
class Filtering:
    def main(self) -> vs.VideoNode:
        ...
        endcard = MaskedDHA(out, rx=1.6, ry=1.6, darkstr=0.02, brightstr=1)
        from vardefunc.aa import upscaled_sraa
        endcard = upscaled_sraa(endcard, 2).rgvs.Repair(endcard, 13)
        dehalo = rfs(out, endcard, [(33927, src.num_frames - 1)])
        out = dehalo
