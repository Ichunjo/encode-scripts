...
JPBD = infos_bd(r'REZERO_2ND_DISC3\BDMV\STREAM\00002', 0, 35485)
...
def do_filter():
    ...
    ref, src, src_nced = [depth(x, 16) for x in [denoise, src, JPBD_NCED.src_cut[1:]]]
    credit_mask = vdf.drm(ref, kernel='bilinear', mthr=65)
    credit = out
    credit = lvf.rfs(credit, core.std.MaskedMerge(credit, ref, credit_mask, 0), [(0, 1521)])
    credit = lvf.rfs(credit, ref, [(1558, 1677)])
    out = credit
