...
JPBD = infos_bd(r'REZERO_2ND_DISC2\BDMV\STREAM\00001', 0, 37639)
...
def do_filter():
    ...
    deband = lvf.rfs(deband, deband_d, [(10335, 10916), (11742, 11882), (12038, 12259),
                                        (12343, 12504), (12634, 13052), (13113, 13351),
                                        (14578, 14663), (15378, 15439), (16125, 16155),
                                        (16184, 16263), (16345, 16687), (16965, 17034),
                                        (28105, 30149)])
    deband = lvf.rfs(deband, deband_e, [(24699, 24889), (25307, 25383)])
    ...
    credit = lvf.rfs(out, core.std.MaskedMerge(out, ref, credit_mask, 0), [(35113, 37569)])
    credit = lvf.rfs(credit, ref, [(37570, src.num_frames-1)])
    out = credit


    src_c, src_ncop = [c.knlm.KNLMeansCL(a=7, h=35, d=0, device_type='gpu') for c in [src, JPBD_NCOP.src_cut]]
...
def do_encode(clip):
    ...
    print('\n\n\nAudio extraction')
    eac3to_args = ['eac3to', JPBD.src, '2:', JPBD.a_src.format(1)]
