...
JPBD = infos_bd(r'REZERO_2ND_DISC1\BDMV\STREAM\00005', 0, 42437)
...
def do_filter():
    ...
    ref, src, src_nced = [depth(x, 16) for x in [denoise, src, JPBD_NCED.src_cut]]
    credit_mask = vdf.drm(ref, kernel='bilinear', mthr=65)
    credit = lvf.rfs(out, core.std.MaskedMerge(out, ref, credit_mask, 0), [(62, 1463)])
    credit = lvf.rfs(credit, ref, [(15202, 15321), (40160, 40279)])
    out = credit


    src_c, src_nced = [c.knlm.KNLMeansCL(a=7, h=35, d=0, device_type='gpu') for c in [src, src_nced]]
    opening_mask = vdf.dcm(out, src_c[EDSTART:EDEND+1], src_nced[:EDEND-EDSTART+1], EDSTART, EDEND, 4, 4).std.Inflate()
    credit = lvf.rfs(out, core.std.MaskedMerge(out, ref, opening_mask), [(EDSTART, EDEND)])
    out = credit
