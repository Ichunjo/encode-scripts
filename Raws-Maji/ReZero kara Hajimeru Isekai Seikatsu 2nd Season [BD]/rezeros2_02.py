...
JPBD = infos_bd(r'REZERO_2ND_DISC1\BDMV\STREAM\00002', 0, 36924)
...
def do_filter():
    ...
    credit = lvf.rfs(out, core.std.MaskedMerge(out, ref, credit_mask, 0), [(0, 3164)])
...
def do_encode(clip):
    ...
    vdf.generate_keyframes(JPBD.src_cut, JPBD.name + '_op_keyframes.txt')
    ...
    if not os.path.isfile(JPBD.output):
        ...
        x265_cmd += f'--qpfile {JPBD.name}_op_keyframes.txt' + ' '
        x265_cmd += '--deblock=-1:-1 --no-sao --no-sao-non-deblock' + ' '
        x265_cmd += f'--zones {OPSTART},{OPSTART+62},b=1.45' + ' '
