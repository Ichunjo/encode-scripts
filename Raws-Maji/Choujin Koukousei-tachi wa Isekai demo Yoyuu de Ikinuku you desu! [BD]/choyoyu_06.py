...
PATH = r'[BDMV] CHOYOYU\CHOYOYU_2\BDMV\STREAM\00006'
SRC = PATH + '.m2ts'
...
def do_encode(filtered):
    ...
    for i in range(1, len(mka.tracks) + 1):
        qaac_args = ['qaac64', A_SRC_CUT.format(i), '-V', '127', '--no-delay', '-o', A_ENC_CUT.format(i)]
    ...
    mkv.add_track(MKVTrack(A_ENC_CUT.format(1), language='jpn', default_track=True))
    mkv.add_track(MKVTrack(A_ENC_CUT.format(2), track_name='Commentary', language='jpn'))
