...
JPBD = infos_bd(r'[BDMV][201223][GNXA-2293][Lapis_Re_LiGHTs][vol.3 Fin]\LAPIS_RE_LIGHTS_3\BDMV\STREAM\00017', 24, 2712)
JPBD_01 = infos_bd(r'[BDMV][201028][GNXA-2291][Lapis_Re_LiGHTs][vol.1]\LAPIS_RE_LIGHTS_1\BDMV\STREAM\00005', 0, 34047)
...
def do_filter():
    ...
    image01[0] = core.resize.Bicubic(image01[0], format=vs.YUV420P8, matrix=1) * src.num_frames
    image01[0] = core.std.AssumeFPS(image01[0], src)

    image02 = core.imwri.Read('1080elements_mv01_02.png', alpha=True)
    image02[0] = core.resize.Bicubic(image02[0], format=vs.YUV420P8, matrix=1) * src.num_frames
    image02[0] = core.std.AssumeFPS(image02[0], src)

    src = lvf.rfs(src, core.std.MaskedMerge(src, image01[0], image01[1]), [(fr_s, 1534)])
    src = lvf.rfs(src, core.std.MaskedMerge(src, image02[0], image02[1]), [(1535, fr_e)])
