...
JPBD = infos_bd(r'[BDMV][201223][GNXA-2293][Lapis_Re_LiGHTs][vol.3 Fin]\LAPIS_RE_LIGHTS_3\BDMV\STREAM\00005', 0, 34049)
...
def do_filter():
    ...
    aa_a = sraa_eedi3(upscale, 13, alpha=0.3, beta=0.5, gamma=40)
    aa_b = sraa_eedi3(aa_a, None, alpha=0.8, beta=0.1, gamma=20)


    mask_aa_b = core.imwri.Read('aa_mask_10_01.png').std.AssumeFPS(aa_b).resize.Bilinear(aa_b.width, aa_b.height, vs.GRAYS, matrix_s='709')
    aa = lvf.rfs(aa_a, core.std.MaskedMerge(aa_a, aa_b, mask_aa_b), [(22792, 22922)])
    ...
    credit = lvf.rfs(credit, core.std.MaskedMerge(credit, ref, rescale_mask, 0),
                     [(opstart, opend), (3506, 3601), (30742, 32879)])
    credit = lvf.rfs(credit, ref, [(12733, 12852)])
    out = credit
