...
JPBD = infos_bd(r'[BDMV] ORESUKI Are you the only one who loves me\ORESUKI OVA\BD_VIDEO\BDMV\STREAM\00001', 0, -24)
JPBD_NCOP = infos_bd(r'[BDMV] ORESUKI Are you the only one who loves me\ORESUKI Volume 4\BD_VIDEO\BDMV\STREAM\00006', 24, -24)
X265 = 'x265'
...
def do_filter():
    ...
    opening_mask = vdf.dcm(out, src_c[opstart:opend+1], ncop[:opend-opstart+1], opstart, opend, 3, 3).std.Convolution([1]*9)

    credit = lvf.rfs(out, core.std.MaskedMerge(out, src, opening_mask), [(opstart, opend)])
    out = credit


    logo_mask = lvf.src('credit_mask_logo.png', out).std.Convolution([1]*9)
    credit = lvf.rfs(out, core.std.MaskedMerge(out, ref, logo_mask), [(20992, 23225)])
