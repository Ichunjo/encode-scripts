...
JPBD = infos_bd(r'[BDMV] ORESUKI Are you the only one who loves me\ORESUKI Volume 2\BD_VIDEO\BDMV\STREAM\00002', 0, -26)
...
def do_filter():
    ...
    decomb = lvf.rfs(out, decomb, [(4807, 4815)])
    ref = decomb
    out = decomb
    ...
    credit = lvf.rfs(out, core.std.MaskedMerge(out, antialias, credit_mask),
                     [(4813, 4920), (33926, src.num_frames-1), (7971, 8288)])
    out = credit
