...
JPBD = infos_bd(r'[BDMV] ORESUKI Are you the only one who loves me\ORESUKI Volume 4\BD_VIDEO\BDMV\STREAM\00001', 0, -26)
...
def do_filter():
    ...
    if out.num_frames < 34046:
        while out.num_frames != 34046:
            out += out[-1]
    opstart, opend = 1774, 3932
    edstart, edend = 30377, 32535
    h = 720
    w = get_w(h)
    ...
    decomb = lvf.rfs(out, decomb, [(3945, 3953)])
    ref = decomb
    out = decomb
    ...
    credit = lvf.rfs(out, core.std.MaskedMerge(out, antialias, credit_mask),
                     [(3951, 4058), (33926, src.num_frames-1), (5270, 5478)])
    out = credit
