...
JPBD = infos_bd(r'[BDMV] ORESUKI Are you the only one who loves me\ORESUKI Volume 3\BD_VIDEO\BDMV\STREAM\00001', 0, -26)
...
def do_filter():
    ...
    decomb = lvf.rfs(out, decomb, [(3337, 3345)])
    ref = decomb
    out = decomb
    ...
    regrain = lvf.rfs(out, regrain, [(31011, 31069)])
    out = regrain
    ...
    credit = lvf.rfs(out, core.std.MaskedMerge(out, antialias, credit_mask),
                     [(3342, 3449), (33926, src.num_frames-1)])
    out = credit
