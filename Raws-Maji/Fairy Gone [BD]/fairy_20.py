...
JPBD = infos_bd(r'[BDMV] Fairy gone\[BDMV][200219][TBR29117D][Fairy gone フェアリーゴーン Vol.7]\BDROM\BDMV\STREAM\00001', 24, -24)
...
def do_filter():
    ...
    # Variables
    eptitle_s, eptitle_e = 2014, 2133
    edstart, edend = 30496, 32654
    preview_s, preview_e = 33805, src.num_frames-1
    h = 720
    w = get_w(h)
    ...
    merged = lvf.rfs(merged, out, [(eptitle_s, eptitle_e), (preview_s, preview_e)])
    out = depth(merged, 16)
    ...
    credit = out
    dehalo_range_a = [(10165, 10284), (15979, 16044), (17775, 17845)]
    dehalo_range_b = [(9353, 9411)]
    credit = lvf.rfs(credit, core.std.MaskedMerge(credit, ref, rescale_mask, 0),
                     [(24, 2013), (18556, 18655), (32679, 32725),
                      (edstart, edend)]
