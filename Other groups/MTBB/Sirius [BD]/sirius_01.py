...
JPBD = infos_bd(r'sirius01', 24, -25)
...
def do_filter():
    ...
    # Variables
    full_zone = [(18629, 18676), (6751, 6846), (33987, src.num_frames-1)] # eyecatch, episode name and next episode
    garbage_zone = [(2011, 2208), (4138, 31193), (33231, 33806)]
    h = 720
    w = get_w(h)
    ...
    edges = lvf.rfs(src, edges, garbage_zone)
    out = depth(edges, 32)


    # Denoise
    ref = hvf.SMDegrain(depth(get_y(out), 16), thSAD=450)
    denoise_a = hybrid_denoise(out, 0.35, 1.4, dict(a=2, d=1), dict(ref=depth(ref, 32)))
    denoise_b = hybrid_denoise(out, 0.80, 4.0, dict(a=2, d=1), dict(ref=depth(ref, 32), profile1='np'))
    denoise = lvf.rfs(denoise_a, denoise_b, [(29651, 29759)])
    out = denoise
    ...
    descale_a = core.descale.Debicubic(luma, w, h, 0, 1)
    descale_b = core.descale.Despline36(luma, w, h)
    descale = lvf.rfs(descale_a, descale_b, garbage_zone)
    ...
    merged = lvf.rfs(merged_b, merged_a, garbage_zone + [(2209, 2584)])
    out = depth(merged, 16)
    ...
    out = lvf.rfs(out, depth(denoise, 16), [(31194, 33230)])
    ...
    dygrain_a = sizedgrn(out, 0.2, 0.05, 1.15, sharp=80, static=False)
    dygrain_b = sizedgrn(out, 0.25, 0.15, 1.5, sharp=80, static=False)
    dygrain = lvf.rfs(dygrain_a, dygrain_b, [(29651, 29759)])
