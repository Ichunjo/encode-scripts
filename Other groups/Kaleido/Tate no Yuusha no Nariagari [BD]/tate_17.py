...
JPBD = infos_bd(r'[BDMV][190626][Tate no Yuusha no Nariagari][Vol.3]\TATE_3_2\BDMV\STREAM\00009', 24, -26)
JPBD_NCOP = infos_bd(r'[BDMV][190524][Tate no Yuusha no Nariagari][Vol.2]\TATE_2_2\BDMV\STREAM\00012', 24, -24)
USBD = infos_bd(r'[BDMV] The Rising of the Shield Hero S01 Part 2\[BDMV] Rising_Shield_Hero_S1P2_D1\BDMV\STREAM\00017', 24, -24)
...
def do_filter():
    ...
    opstart, opend = 1678, 3848
    edstart, edend = 30557, 32715
    src = JPBD.src_cut
    src += src[-1]
    src = depth(src, 16)
    ...
    grain = lvf.rfs(grain, grain_original, [(214, 499), (23851, 24119)])
...
def do_encode(filtered):
    ...
    print('\n\n\nVideo encoding')
    vdf.encode(filtered[-1], X264, JPBD.output, **X264_ARGS)
