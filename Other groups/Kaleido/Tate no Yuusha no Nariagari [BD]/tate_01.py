...
JPBD = infos_bd(r'[BDMV][190424][Tate no Yuusha no Nariagari][Vol.1]\TATE_1_1\BDMV\STREAM\00009', 24, -24)
JPBD_NCOP = infos_bd(r'[BDMV][190424][Tate no Yuusha no Nariagari][Vol.1]\TATE_1_2\BDMV\STREAM\00013', 24, -24)
USBD = infos_bd(r'[BDMV] The Rising of the Shield Hero S01 Part 1\[BDMV] Rising_Shield_Hero_S1P1_D1\BDMV\STREAM\00014', 24, -24)
...
def do_filter():
    ...
    def _perform_masks_credit(path: Path) -> List:
    ...
    src_c, src_ncop = [c.knlm.KNLMeansCL(a=6, h=20, d=0, device_type='gpu')
                       for c in [src[opstart:opend+1], JPBD_NCOP.src_cut[:opend-opstart+1]]]
    credit_mask = vdf.dcm(src, src_c, src_ncop, opstart, opend, 2, 2).std.Deflate()
    ...
    final = lvf.rfs(final, src, [(4891, 5010)])
