...
class Filtering():  # noqa
    ...
    @staticmethod
    def crop_and_fix(clip: vs.VideoNode, ref: vs.VideoNode, top: int, bottom: int, fix_edges: bool = True) -> vs.VideoNode:  # noqa
        clip = clip.std.Crop(top=top, bottom=bottom)

        if fix_edges:
            ref = ref.std.Crop(top=top, bottom=bottom)
            mask = clip.std.BlankClip(format=vs.GRAY16, color=65535)
            mask = vdf.mask.region_mask(mask, top=4, bottom=4).std.Invert().std.Inflate()

            fixedg = core.std.MaskedMerge(clip, ref, mask)
            fixedg = core.edgefixer.ContinuityFixer(fixedg, 0, [1, 1, 1], 0, [1, 1, 1])
        else:
            fixedg = clip

        if fixedg.height != 816:
            fixedg = core.fmtc.resample(fixedg, h=816, kernel='blackmanminlobe')

        return core.std.AddBorders(fixedg, top=132, bottom=132, color=[((16 + 25) / 2) * 2**8, 128 << 8, 128 << 8])
