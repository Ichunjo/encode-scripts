...
JPBD = infos_bd(r'[BDMV][200904][CYGX-00002][Princess Connect! Re_Dive Vol.2]\BD_VIDEO\BDMV\STREAM\00002', 0, -26)
...
def do_filter():
    ...
    credit = lvf.rfs(credit, core.std.MaskedMerge(credit, src, rescale_mask, 0),
                     [(opstart+425, opstart+698), (33972, src.num_frames-1), (2645, 2739)])
    credit = lvf.rfs(credit, core.std.MaskedMerge(credit, src, creditless_mask, 0),
                     [(opstart, opend), (edstart, edend)])
    credit = lvf.rfs(credit, src, [(8906, 8965), (17568, 17615), (21432, 21503), (26917, 26964)])
    out = credit
