...
JPBD_NCOP = FileInfo(VPath(*paths['ncops'][0]), (0, -24), preset=[PresetBD])

BDMV_FOLDER = VPath(paths['first_press_path'], paths['first_press_vol1'])
...
class Filtering:
    ...
    def main(self, debug: DebugOutput = DEBUG) -> vs.VideoNode:
        ...
        dehalo = MaskedDHA(out, 1.1, 1.1, 0, 1.)
        out = dehalo
        ...
        nc_mask = diff_creditless_mask(
            out, depth(mean, 16)[OPSTART:OPEND], JPBD_NCOP.clip_cut, OPSTART,
        ...
        credit = remap_rfs(credit, core.std.MaskedMerge(credit, ref, rescale_mask), [(3942, 5917)])
        credit = remap_rfs(credit, core.std.MaskedMerge(credit, ref, nc_mask), (OPSTART, OPEND))
        out = credit
        ...
        # deband_b = placebo_deband(out, threshold=5.5, iterations=2)
        deband = remap_rfs(deband, deband_b, [(337, 492)])
