...
JPBD_NCOP = FileInfo(
    BDMV_FOLDER / SUB_BDMV_FOLDER / 'BDMV/STREAM/00003.m2ts',
    (0, -24)
)
JPBD_NCED = FileInfo(
...
class Filtering:
    ...
    def main(self, src: vs.VideoNode = JPBD.clip_cut, debug: DebugOutput = DEBUG) -> vs.VideoNode:
        ...
        up1 = fsrcnnx_upscale(descale, height=descale.height*2, shader_file=Shader.FSRCNNX_56_16_4_1)
        ...
        # rescale_mask = diff_rescale_mask(pre, kernel=Bilinear(), thr=80/219)
        # rescale_mask = region_mask(rescale_mask, 600, 600, 0, 0).std.Expr('x 65535 *', vs.GRAY16)
        bar_mask = core.std.Expr(
            [region_mask(out.std.BlankClip(format=vs.GRAY16, color=65535), 632, abs(650 - 1920)),
             region_mask(out.std.BlankClip(format=vs.GRAY16, color=65535), 1271, abs(1287 - 1920))],
            'x y +'
        ).std.BoxBlur(0, 1, 1, 0, 0)
        # debug <<= bar_mask
        ...
        credit = replace_ranges(credit, core.std.MaskedMerge(credit, ref, bar_mask), (23863, 23941))
        credit = replace_ranges(credit, core.std.MaskedMerge(credit, ref, nc_mask), [(OPSTART, OPEND), (EDSTART, EDEND)])
        out = credit
