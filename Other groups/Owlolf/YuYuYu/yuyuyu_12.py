...
JPBD_NCOP = FileInfo(VPath(*paths['ncops'][2]), (0, -24), preset=[PresetBD])
JPBD_NCED = FileInfo(VPath(*paths['nceds'][4]), (0, -24), preset=[PresetBD])

BDMV_FOLDER = VPath(paths['first_press_path'], paths['first_press_vol6'])
...
OPSTART, OPEND = 936, 3093
EDSTART, EDEND = 32075, None

# import lvsfunc

DEBUG = DebugOutput(
    JPBD.clip_cut,
    # JPBD.clip_cut[OPSTART:OPEND],
    # JPBD_NCOP.clip_cut
    # JPBD2.clip_cut
...
class Filtering:
    ...
    def main(self, debug: DebugOutput = DEBUG) -> vs.VideoNode:
        mean = core.average.Mean([depth(c, 32) for c in [JPBD.clip_cut, JPBD2.clip_cut]], preset=3)
        ...
        # den_y_b = Denoise.bm3d(get_y(out), [0.2, 0, 0], radius=1, profile='fast')
        # den_y = remap_rfs(den_y, den_y_b, [(3564, 3639)])
        ...
        # black = core.std.BlankClip(ncop_mask)
        # white = core.std.BlankClip()
        # nc_mask = core.std.Expr((ncop_mask, nced_mask), 'x y +').std.Convolution([1]*9)
        # nc_mask = remap_rfs(
        #     nc_mask, nc_mask,
        #     [(OPSTART + 305, OPSTART + 340), (OPSTART + 423, OPSTART + 482)]
        # )
        # ncop_mask = ncop_mask.std.Convolution([1]*9)

        # debug <<= nced_mask
        # debug <<= rescale_mask[OPSTART:]
        credit = out
        ref = depth(denoise, 16)
        credit = remap_rfs(credit, core.std.MaskedMerge(credit, ref, rescale_mask), [(OPSTART, OPEND)])
        # credit = remap_rfs(credit, core.std.MaskedMerge(credit, ref, nc_mask), [(OPSTART, OPEND), (EDSTART, EDEND)])
        credit = remap_rfs(credit, core.std.MaskedMerge(credit, ref, nced_mask), [(EDSTART, EDEND)])
        ...
        # deband_b = dumb3kdb(out, threshold=17, grain=24)
        # deband_b = placebo_deband(out, threshold=6.5, iterations=2)
        # deband = remap_rfs(deband, deband_b, [(3564, 3639)])
