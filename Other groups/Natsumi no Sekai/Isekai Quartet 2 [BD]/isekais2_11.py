...
JPBD = infos_bd(r'Isekai Quartet S2\Vol2\BDROM\BDMV\STREAM\00009', 0, -24)
...
def do_filter():
    ...
    src = JPBD.src_cut
    src = depth(src, 32)
    edstart, edend = 14970, src.num_frames-1
    ...
    endcard = endcard_source(r'endcards\yande.re 620673 ainz_ooal_gown armor cleavage dress gym_uniform heels isekai_quartet natsuki_subaru overlord satou_kazuma toyota_saori uniform wallpaper youjo_senki.jpg', src)
    endcard_ar = endcard.width/endcard.height
    endcard_ev = bool(endcard.format.name == 'YUV420P8')

    endcard = depth(endcard, 16)
    endcard = gf.MaskedDHA(endcard, rx=1.4, ry=1.4, darkstr=0.3, brightstr=1.0, maskpull=48, maskpush=140)
