...
JPBD = infos_bd(r'Isekai Quartet S2\Vol2\BDROM\BDMV\STREAM\00006', 0, -24)
...
def do_filter():
    ...
    endcard = endcard_source(r'endcards\yande.re 615209 ass bikini_top crossover horns isekai_quartet neko overlord pack_(re_zero) pointy_ears satou_kazuma swimsuits tagme uniform wet wings youjo_senki.jpg', src)
    endcard_ar = endcard.width/endcard.height
    endcard_ev = bool(endcard.format.name == 'YUV420P8')

    endcard = depth(endcard, 16)
    endcard = gf.MaskedDHA(endcard, rx=0.95, ry=0.95, darkstr=0.3, brightstr=1.0, maskpull=48, maskpush=140)
