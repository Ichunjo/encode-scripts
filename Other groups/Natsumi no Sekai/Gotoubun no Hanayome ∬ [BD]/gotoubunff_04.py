...
JPBD = FileInfo(r'[BDMV][210421][Gotoubun no Hanayome ∬][Vol.2]\BDMV\STREAM\00005.m2ts', 0, -24,
...
class Filtering():
    def main(self) -> vs.VideoNode:
        ...
        bdweb = lvf.rfs(bdweb, src_web_y, [(24, 1181), (32162, 33361), (33939, 33994), (34109, 34231)])
        ...
        diff_mask1 = Mask.diff_mask((src, src_cru), brz=4750.0)
        diff_mask2 = Mask.diff_mask((src, src_cru), brz=4200.0)
        bdchanges = out
        bdchanges = lvf.rfs(bdchanges, core.std.MaskedMerge(out, dering_src, diff_mask1),
                            [(4932, 5045), (6075, 6296), (7816, 7883), (9806, 10027),
                             (15031, 15132), (18858, 19073), (22053, 22364), (22545, 22616),
                             (26636, 26683)])
        bdchanges = lvf.rfs(bdchanges, core.std.MaskedMerge(out, dering_src, diff_mask2),
                            [(1728, 1865), (12196, 12405)])
        bdchanges = lvf.rfs(bdchanges, dering_src,
                            [(9450, 9463), (16348, 16395)])
        # return bdchanges
        out = bdchanges
