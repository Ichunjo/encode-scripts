...
JPBD = src_file(
    r'[BDMV][アニメ] ブギーポップは笑わない (2019) [BD Vol.1-5][FIN]\BOOGIEPOP_AND_OTHERS_4\BDMV\STREAM\00000.m2ts',
...
def fix_chair(
    clip: vs.VideoNode,
    good_trim: tuple[int, int] = (16758, 16818),
    replace_trim: tuple[int, int] = (10196, 10304)
) -> vs.VideoNode:
    replace = clip[slice(*replace_trim)]
    good = clip[slice(*good_trim)].std.Loop(2)[:replace.num_frames]
    white = replace.std.BlankClip(format=vs.GRAYS, color=1)
    mask1 = region_rel_mask(white, 679, 1222, 197, 374)
    mask2 = region_rel_mask(white, 1242, 658, 197, 374)
    mask3 = region_rel_mask(white, 797, 664, 144, 900)
    mask = ExprOp.MAX(mask1, mask2, mask3)
    good = core.std.MaskedMerge(replace, good, mask)
    return insert_clip(clip, good, replace_trim[0])
...
def main() -> vs.VideoNode:
    ...
    rs.doubled = replace_ranges(doubled, aa, [(5311, 5407), ], exclusive=True)
    ...
    # Scenefiltering
    db = replace_ranges(db, db_strg, [(9065, 9125), (9980, 10196), (14542, 14620), (16437, 16509),
                                      (22297, 22369), (23173, 23293), (23482, 23506)], exclusive=True)
    db = replace_ranges(db, db_strg2, [(11246, 11372), (24164, 24240), (24306, 24342), (31025, 31047)], exclusive=True)
    white = db.std.BlankClip(format=vs.GRAY16, color=65535)
    db = replace_ranges(db, core.std.MaskedMerge(db, db_strg2, region_rel_mask(white, 888, 0, 0, 0)), (24270, 24306))
    # 
