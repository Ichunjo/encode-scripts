...
from vskernels import Bicubic, Bilinear, FFmpegBicubic, Gaussian, Hermite, KernelT
from vsmasktools import Morpho, XxpandMode, adg_mask, diff_creditless, dre_edgemask, region_rel_mask
from vsmuxtools import FFV1, LosslessPreset, Setup, src_file
from vspreview.api import is_preview
from vsrgtools import box_blur
from vstools import DitherType, core, depth, finalize_clip, get_neutral_value, join, replace_ranges, set_output, split
...
JPBD = src_file(
    r'[BDMV][アニメ] ブギーポップは笑わない (2019) [BD Vol.1-5][FIN]\BOOGIEPOP_AND_OTHERS_1\BDMV\STREAM\00000.m2ts',
    force_lsmas=True, trim=(24, 2182)
    # force_lsmas=True, trim=(24, -24)
)

NCOP = src_file(
    r'[BDMV][アニメ] ブギーポップは笑わない (2019) [BD Vol.1-5][FIN]\BOOGIEPOP_AND_OTHERS_1\BDMV\STREAM\00003.m2ts',
    force_lsmas=True, trim=(24, -24 + OPEND - OPSTART - 2160)
)
...
def main() -> vs.VideoNode:
    ...
    y, u, v = split(src_16)
    y = core.edgefixer.Continuity(y, 4, 4, 4, 4)
    u = core.fb.FillBorders(u, 2, 2, 2, 2)
    v = core.fb.FillBorders(v, 2, 2, 2, 2)
    ...
    # Descale process
    op_credit_mask = diff_creditless(src, NCOP.init_cut(), 0.81, OPSTART, prefilter=True)
    op_credit_mask = region_rel_mask(op_credit_mask, 20, 20, 20, 20)
    ...
    assert upscaled.format
    upscaled = core.std.MaskedMerge(upscaled, upscaled2, depth(op_credit_mask, upscaled.format.bits_per_sample))
    out = upscaled
...
if is_preview():
    ...
    enc = FFV1(LosslessPreset.SPEED, ensure_props=False)
