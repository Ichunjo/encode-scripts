...
JPBD = src_file(
    r'[BDMV][アニメ] ブギーポップは笑わない (2019) [BD Vol.1-5][FIN]\BOOGIEPOP_AND_OTHERS_3\BDMV\STREAM\00001.m2ts',
    force_lsmas=True, trim=(24, -23)
)

EPISODE = '08'
(OPSTART, OPEND) = (0, 2158)
(EDSTART, EDEND) = (31886, JPBD.src_cut.num_frames)
...
def main() -> vs.VideoNode:
    ...
    # Scenefiltering
    db = replace_ranges(db, db_strg2, [(13079, 13193)], exclusive=True)
    # 
    ...
    finalized = finalized.std.FreezeFrames(finalized.num_frames - 1, finalized.num_frames - 1, finalized.num_frames - 2)
