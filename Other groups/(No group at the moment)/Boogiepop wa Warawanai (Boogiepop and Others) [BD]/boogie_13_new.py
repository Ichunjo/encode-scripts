...
from vstools import (
    DitherType, KwargsT, core, depth, finalize_clip, get_neutral_value, insert_clip, join,
...
JPBD = src_file(
    r'[BDMV][アニメ] ブギーポップは笑わない (2019) [BD Vol.1-5][FIN]\BOOGIEPOP_AND_OTHERS_4\BDMV\STREAM\00003.m2ts',
...
def main() -> vs.VideoNode:
    ...
    # Scenefiltering
    db = replace_ranges(db, db_weak, [(869, 959), (1266, 1329)], exclusive=True)
    db = replace_ranges(db, db_strg, [(11885, 11930), (12425, 12612), (19120, 19169), (19201, 19320), (20439, 20727),
                                      (26372, 26471), (28357, 28584), (29292, 29346), (31397, 31517)], exclusive=True)
    db = replace_ranges(db, db_strg2, [(5512, 5560),], exclusive=True)
