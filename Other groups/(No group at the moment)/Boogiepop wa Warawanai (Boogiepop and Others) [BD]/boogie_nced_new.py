...
from vstools import DitherType, core, depth, finalize_clip, get_neutral_value, join, set_output
...
def main() -> vs.VideoNode:
    ...
    staticnoise_mask = norm_expr((staticnoise_mask, join(depth(edgemask, 32), blank)), 'x y + 0.0 1.0 clamp')
    ...
    db_weak = Placebo.deband(out, radius=16, thr=[1.3, 2.6], iterations=12, grain=[0, 0.1])
    db_strg = Placebo.deband(out, radius=16, thr=[2.5, 4.4], iterations=16, grain=[0, 0.5])
...
if is_preview():
    ...
    enc = x265('boogiepop_common/x265_settings', qp_clip=NCED.init_cut(), quiet_merging=False)
    ...
    audio = do_audio(NCED, encoder=Opus())
