...
from vsaa import based_aa, upscaled_sraa
...
JPBD = src_file(
    r'[BDMV][アニメ] ブギーポップは笑わない (2019) [BD Vol.1-5][FIN]\BOOGIEPOP_AND_OTHERS_4\BDMV\STREAM\00001.m2ts',
...
def main() -> vs.VideoNode:
    ...
    rs.doubled = replace_ranges(doubled, aa, [(21145, 21496), ], exclusive=True)
    out = rs.upscale.with_chroma()
    # set_output(out)
    
    uaa = upscaled_sraa(out, 2, downscaler=Hermite(), planes=[1, 2])
    uaa = replace_ranges(out, uaa, [(11492, 11564), (13347, 13425)], exclusive=True)
    out = uaa
    ...
    # Scenefiltering
    db = replace_ranges(db, db_weak, [(17013, 17202),], exclusive=True)
    db = replace_ranges(db, db_strg2, [(26515, 26626),], exclusive=True)
    # 
