...
JPBD = src_file(
    r'[BDMV][アニメ] ブギーポップは笑わない (2019) [BD Vol.1-5][FIN]\BOOGIEPOP_AND_OTHERS_1\BDMV\STREAM\00002.m2ts',
...
def main() -> vs.VideoNode:
    ...
    def aa(clip: vs.VideoNode) -> vs.VideoNode:
        aa = upscaled_sraa(clip)
        return replace_ranges(clip, aa, [(9136, 9223), (10813, 10897), (11338, 11436)], exclusive=True)
    ...
    # Scenefiltering
    db = replace_ranges(db, db_weak, (6155, 6263), exclusive=True)
    db2 = Placebo.deband(out, radius=25, thr=3.5, iterations=16)
    db = replace_ranges(db, db2, (16746, 16884), exclusive=True)
