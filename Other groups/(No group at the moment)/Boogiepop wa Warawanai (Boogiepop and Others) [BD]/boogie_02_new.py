...
from betterdescaletarget import cambi_mask, denoise
from lvsfunc import get_match_centers_scaling
from vodesfunc import DescaleTarget, Waifu2x_Doubler
...
from vskernels import Bicubic, FFmpegBicubic, Hermite, KernelT
from vsmasktools import Morpho, PrewittTCanny, XxpandMode, adg_mask, diff_creditless_oped, dre_edgemask, region_rel_mask
...
JPBD = src_file(
    r'[BDMV][アニメ] ブギーポップは笑わない (2019) [BD Vol.1-5][FIN]\BOOGIEPOP_AND_OTHERS_1\BDMV\STREAM\00001.m2ts',
...
def main() -> vs.VideoNode:
    ...
    def dehalo_aa(clip: vs.VideoNode) -> vs.VideoNode:
        dehalo = fine_dehalo(clip)
        dehalo = replace_ranges(clip, dehalo, (6160, 6394), exclusive=True)
        aa = upscaled_sraa(dehalo)
        mask = PrewittTCanny.edgemask(get_y(clip), 0.2, multi=5, clamp=(0, 1)).std.Maximum().std.Minimum()
        mask = region_rel_mask(mask, 0, 0, 414, 0)
        aa = core.std.MaskedMerge(dehalo, aa, mask)
        return replace_ranges(clip, aa, [(30250, 30322)], exclusive=True)
    ...
    dt1 = DescaleTarget(
        **native_res,
        ...
        line_mask=False, credit_mask=oped_credit_mask,
        do_post_double=dehalo_aa
    )
    upscaled = dt1.get_upscaled(out)
    out = upscaled
    ...
    def sharp_grain_kernel(sharp: float) -> KernelT:
        b = sharp / -50 + 1
        c = (1 - b) / 2
        return Bicubic(b, c)
    ...
    # Scenefiltering
    db2 = Placebo.deband(out, radius=22, thr=3.2, iterations=18)
    db = replace_ranges(db, db2, [(27274, 27514), (14989, 15037)], exclusive=True)
