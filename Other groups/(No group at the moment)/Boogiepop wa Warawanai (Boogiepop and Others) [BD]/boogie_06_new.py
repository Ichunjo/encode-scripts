...
from vskernels import Bicubic, Bilinear, FFmpegBicubic, Hermite, Kernel
from vsmasktools import (
    FDoGTCanny, Morpho, PrewittStd, XxpandMode, adg_mask, diff_creditless_oped, dre_edgemask,
    region_rel_mask
)
...
from vstools import (
    DitherType, core, depth, finalize_clip, get_neutral_value, get_y, insert_clip, join,
...
JPBD = src_file(
    r'[BDMV][アニメ] ブギーポップは笑わない (2019) [BD Vol.1-5][FIN]\BOOGIEPOP_AND_OTHERS_2\BDMV\STREAM\00002.m2ts',
...
def main() -> vs.VideoNode:
    ...
    doubled = rs.doubled
    dehalo = fine_dehalo(doubled)
    dehalo = replace_ranges(doubled, dehalo, [(11669, 12107), ], exclusive=True)
    aa = upscaled_sraa(dehalo, 1.5)
    aa = core.std.MaskedMerge(dehalo, aa, PrewittStd.edgemask(get_y(dehalo)))
    rs.doubled = replace_ranges(
        dehalo, aa,
        [(25484, 25553), (25790, 25883), (28580, 28613), (19080, 20313)],
        exclusive=True
    )
    out = rs.upscale.with_chroma()
    aachroma = upscaled_sraa(out, 2, planes=[1, 2])
    aachroma = replace_ranges(out, aachroma, (19080, 20313), exclusive=True)
    out = aachroma
    ...
    db_strg2 = Placebo.deband(out, radius=20, thr=[2.2, 5.5], iterations=16)
    ...
    # Scenefiltering
    db = replace_ranges(db, db_weak, [(12461, 12602), (12871, 13001), (15867, 16079)], exclusive=True)
    db = replace_ranges(db, db_strg2, [(13130, 13163), (13319, 13441)], exclusive=True)
    # 
    deband = core.std.MaskedMerge(db, out, edgemask)
    # 
    db_strg2 = Placebo.deband(out, radius=10, thr=2.8, iterations=16)
    edgemask2 = combine(Bilinear.resample(FDoGTCanny.edgemask(out), vs.YUV444P16), split_planes=True)
    deband = replace_ranges(deband, core.std.MaskedMerge(db_strg2, out, edgemask2), [(13001, 13040)], exclusive=True)
    # Scenefiltering
    out = deband
