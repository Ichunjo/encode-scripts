...
JPBD = infos_bd(r'[BDMV][200610][Magia Record][Vol.3]\BD_VIDEO\BDMV\STREAM\00002', 0, -48)
BLANK = 'blank.wav'
X264_ARGS = dict(
...
def do_filter():
    ...
    deband_c = core.neo_f3kdb.Deband(deband_a, 20, 64, 64, 64, 24, 0, sample_mode=2, keep_tv_range=True)

    deband = lvf.rfs(deband_a, deband_b, [(edstart+1870, edstart+1900)])
    deband = lvf.rfs(deband, core.std.MaskedMerge(deband_c, deband_a, to_gray(lvf.src('masks/magia_07_mask_01.png'), src)),
                     [(6729, 6865)])
    ...
    endcard = _perform_endcard('[BDMV][200610][Magia Record][Vol.3]/Scans/endcard7_front_descreen.png', src)
    endcard_length = 119
...
def do_encode(filtered):
    ...
    vdf.subprocess.run(qaac_args, text=True, check=True, encoding='utf-8')
    opus_args = ['ffmpeg', '-i', 'blank_cut_track_1.wav', '-c:a', 'libopus', '-b:a', '192k', '-y', 'blank_cut.opus']
    vdf.subprocess.run(opus_args, text=True, check=True, encoding='utf-8')

    print('\nFinal muxing')
    mkv_args = ['mkvmerge', '-o', JPBD.name + '_aac.mkv',
                '--language', '0:jpn', JPBD.output,
                '--language', '0:jpn', JPBD.a_enc_cut.format(1) + '.m4a', '+', 'blank_cut.m4a',
                '--language', '0:jpn', '--track-name', '0:Commentary', JPBD.a_enc_cut.format(2) + '.m4a', '+', 'blank_cut.m4a',
    ...
    mkv_args = ['mkvmerge', '-o', JPBD.name + '_opus.mkv',
                '--language', '0:jpn', JPBD.output,
                '--language', '0:jpn', JPBD.a_enc_cut.format(1) + '.opus', '+', 'blank_cut.opus',
                '--language', '0:jpn', '--track-name', '0:Commentary', JPBD.a_enc_cut.format(2) + '.opus', '+', 'blank_cut.opus',
                '--chapter-language', 'jpn', '--chapters', JPBD.chapter
                ]
    vdf.subprocess.run(mkv_args, text=True, check=True, encoding='utf-8')
