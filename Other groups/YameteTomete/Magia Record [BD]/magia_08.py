...
JPBD = infos_bd(r'[BDMV][200805][Magia Record][Vol.4]\BD_VIDEO\BDMV\STREAM\00001', 0, -48)
X264_ARGS = dict(
...
def do_filter():
    ...
    def _perform_endcard(path: str, ref: vs.VideoNode)-> vs.VideoNode:
        ...
        endcard = core.std.CropRel(endcard, left=0, top=10, right=22, bottom=21)
    ...
    deband_c = core.neo_f3kdb.Deband(deband_a, 17, 64, 42, 42, 12, 0, sample_mode=4, keep_tv_range=True)

    deband = lvf.rfs(deband_a, deband_b, [(edstart+1870, edstart+1900)])
    deband = lvf.rfs(deband, deband_c, [(18140, 18211)])

    grain = kgf.adaptive_grain(deband, 0.25, luma_scaling=10)


    endcard = _perform_endcard('[BDMV][200805][Magia Record][Vol.4]/Scans/endcard8_front_descreen.png', src)
    endcard_length = 119
