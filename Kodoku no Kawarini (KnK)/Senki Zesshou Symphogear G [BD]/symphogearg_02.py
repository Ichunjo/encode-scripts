...
def infos_bd(path, frame_start, frame_end) -> InfosBD:
    src = path + '.m2ts'
    src_clip = core.lsmas.LWLibavSource(src, ff_loglevel=4)
...
def infos_bd_alt(path, frame_start, frame_end) -> InfosBD:
    src = path + '.mkv'
    src_clip = core.ffms2.Source(src)
    src_cut = src_clip[frame_start:frame_end]
    a_src = path + '.wav'
    a_src_cut = path + '_cut_track_{}.wav'
    a_enc_cut = path + '_track_{}.m4a'
...
JPBD_alt = infos_bd_alt(r'戦姫絶唱シンフォギアＧ\[BDMV][131002] 戦姫絶唱シンフォギアG 1\KIXA_90350\BDMV\STREAM\00004', 0, -24)
