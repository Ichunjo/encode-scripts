...
HENSHIN = infos_bd(r'戦姫絶唱シンフォギアＧ\[BDMV][140108] 戦姫絶唱シンフォギアG 4\KIXA_90353\BDMV\STREAM\00005', 0, -24)
...
def do_filter():
    ...
    # Fix black borders for henshins
    start, end = 26596, 27387

    # Good length for the three henshins
    hibiki = HENSHIN.src_cut[912:1716]
    ...
    tsubasa = src.std.BlankClip(length=8, color=[235, 128, 128]) + HENSHIN.src_cut[2178:2738]
    tsubasa = core.std.DuplicateFrames(tsubasa, range(10, 20))
    tsubasa = core.std.DuplicateFrames(tsubasa, range(196, 203))
    tsubasa = core.std.DuplicateFrames(tsubasa, [443]*3)

    yukine = HENSHIN.src_cut[3240:3828]
