...
JPBD = infos_bd_alt(r'戦姫絶唱シンフォギアＧ\[BDMV][140108] 戦姫絶唱シンフォギアG 4\KIXA_90353\BDMV\STREAM\00004', 0, -24)
...
def do_filter():
    ...
    opstart, opend = 720, 3116
    edstart, edend = 31888, src.num_frames - 1
    full_stuff = [(3118, 3224), (15699, 15853)]
    ...
    deband = lvf.rfs(deband, deband_d, [(3117, 3224)])
...
def do_encode(clip: vs.VideoNode)-> None:
    ...
    if not os.path.exists(JPBD.output):
        ...
        print("Encoder command: ", " ".join(x264_args), "\n")
        process = subprocess.Popen(x264_args, stdin=subprocess.PIPE)
        print('cccccccccccc')
        clip.output(process.stdin, y4m=True, progress_update=lambda value, endvalue:
                    print(f"\rVapourSynth: {value}/{endvalue} ~ {100 * value // endvalue}% || Encoder: ", end=""))
        process.communicate()
